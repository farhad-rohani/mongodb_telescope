...
class DatabaseEntriesRepository implements Contract, ClearableRepository, PrunableRepository, TerminableRepository
{
     ...
     * @return void
     */
    public function __construct(string $connection =null, int $chunkSize = null)
    {
        $this->connection = $connection??config('telescope.storage.database.connection');
    ...
    public function store(Collection $entries)
    {
        ...
        $count = $table->count();
        if ($count) {
            $lastEntry = $table->orderByDesc('sequence')->first();
            if ($lastEntry && isset($lastEntry['sequence'])) {
                $index = $lastEntry['sequence'] + 1;
            } else {
                $index = $count + 1;
            }
        } else {
        ...
        $chunks = $entries->chunk($this->chunkSize);
    ...
    protected function storeTags(Collection $results)
    {
       $results->chunk($this->chunkSize)->each(function ($chunked) {
            $values = $chunked->flatMap(function ($tags, $uuid) {
            ...
            if (count($values)) {
                $this->table('telescope_entries_tags')->insert($values);
            }
        });
    }
